...
- Server creates JWT and sends it to the client
- Client stores that token and sends it in all subsequent HTTP requests in the header
...
The general steps for adding PyJWT

- Install PyJWT
- Add a User class to models.py
- Add add login route
- Add a function to identify the user from a token
- Secure endpoints (routes) use the `@jwt_required` decorator
...
Add the following to the `routes.py` (or `api_routes.py` if you already created BluePrints).
